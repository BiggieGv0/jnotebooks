...
def data_loading(datapath):
    data = pandas.read_csv(datapath, index_col=2, parse_dates=[2, 4], date_parser=poi_gen._pdt)
    data['MAEf'] = data['MAE'].apply(lambda v: float(v[:-1]))
    data['MAEp'] = data['Price'] * (data['MAEf'] / 100)
    data['MFEf'] = data['MFE'].apply(lambda v: float(v[:-1]))
    data['MFEp'] = data['Price'] * (data['MFEf'] / 100)
...
def get_d(ttype, path, tf, wsl, **kwa):
    ...
    r2 = [(d[wsl].o, [b.o for b in d]) for d in r1]
...
def plot_poi(wsl, wsr, long_data, short_data):
    ...
    plt.grid(True)
    plt.plot(range(-wsl, wsr), long_data, c='g', marker='o')
    plt.subplot(212)
    plt.xlabel('Bars Holded')
    plt.ylabel('P/L')
    plt.title('POI of Shorts')
    plt.grid(True)
    plt.plot(range(-wsl, wsr), short_data, c='r', marker='o')
